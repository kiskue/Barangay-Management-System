...
namespace BMS
{
    public partial class BuildingPermit : System.Web.UI.Page
    {
        ...
        private void BindGridView()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT DID,FullName,Email,Number,Address,Intention,Age,Build,Date FROM BuildingPermit", con))
                {
        ...
        protected void Edit(object sender, EventArgs e)
        {
            ...
            Intention.Text = row.Cells[4].Text.Trim();
            Age.Text = row.Cells[5].Text.Trim();
            Build.Text = row.Cells[6].Text.Trim();
        ...
        protected void Delete(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("Delete From BuildingPermit WHERE DID = @DID", con))
                {
                    cmd.CommandType = CommandType.Text;
        ...
        protected void Save(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("UPDATE BuildingPermit SET FullName = @FullName, Email = @Email, Number=@Number,Intention=@Intention,Age=@Age,Build=@Build, Date=@Date WHERE DID = @DID", con))
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@DID", lblId.Text.Trim());
                    cmd.Parameters.AddWithValue("@FullName", Name.Text.Trim());
                    cmd.Parameters.AddWithValue("@Email", Email.Text.Trim());
                    cmd.Parameters.AddWithValue("@Number", Number.Text.Trim());
                    cmd.Parameters.AddWithValue("@Intention", Intention.Text.Trim());
                    cmd.Parameters.AddWithValue("@Age", Age.Text.Trim());
                    cmd.Parameters.AddWithValue("@Build", Build.Text.Trim());
        ...
        protected void btndone(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("Delete From BuildingPermit WHERE Email = @Email", con))
                {
                    cmd.CommandType = CommandType.Text;
